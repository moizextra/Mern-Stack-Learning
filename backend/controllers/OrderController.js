...
exports.getSingleOrder = asyncWrapper(async (req, res, next) => {
    const order = await Order.findById(req.params.id).populate(
        'user',
        'name email'
    ); // Here What is happining is that we have user field in order i will go in user model and popultae name and email from AS SAID IN CODE
    if (!order) {
...
exports.UpdateOrder = asyncWrapper(async (req, res, next) => {
    ...
    if (order.OrderStatus == 'Delivered') {
        return next(
            new ErrorHander('You have Already Delivered the Order', 400)
        );
    }
...
const updateStock = asyncWrapper(async (id, quantity) => {
